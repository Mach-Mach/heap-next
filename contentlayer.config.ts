...
export const Article = defineDocumentType(() => ({
  ...
  fields: {
    ...
    arx_url: { type: 'string', required: true },
    images: { type: 'json' },
    authors: { type: 'list', of: { type: 'string' }, default: [] },
    affiliations_aligned: { type: 'list', of: { type: 'string' }, default: [] },
...
export default makeSource({
  contentDirPath: 'data',
  documentTypes: [Article, Authors, Post, Newspost],
  mdx: {
    cwd: process.cwd(),
    remarkPlugins: [
      remarkExtractFrontmatter,
  ...
  onSuccess: async (importData) => {
    const { allArticles, allPosts, allNewsposts } = await importData()
    createTagCount(allArticles)
    createSearchIndex(allArticles)
    createSearchIndex(allPosts)
    createSearchIndex(allNewsposts)
  },
})
